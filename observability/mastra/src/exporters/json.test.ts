...
describe('JsonExporter', () => {
  ...
  describe('query methods', () => {
    it('should get spans by SpanType', async () => {
      ...
      await exporter.exportTracingEvent(createEvent(TracingEventType.SPAN_ENDED, { ...agentSpan, endTime: new Date() }));
    ...
    it('should get root spans only', async () => {
      ...
      await exporter.exportTracingEvent(createEvent(TracingEventType.SPAN_ENDED, { ...childSpan, endTime: new Date() }));
  ...
  describe('lifecycle validation warnings', () => {
    it('should warn when span starts twice', async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining('started twice'),
      );
    });
    ...
    it('should warn when normal span ends without starting', async () => {
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        expect.stringContaining('ended without starting'),
      );
    });
  });
});
